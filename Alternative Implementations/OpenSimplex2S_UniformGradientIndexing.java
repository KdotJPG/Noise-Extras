/**
 * OpenSimplex2S noise with more uniform gradient vector indexing.
 */

public class OpenSimplex2S_UniformGradientIndexing {
    ...
    private static final int N_GRADIENTS_4D_BASE_EXPONENT = 5;
    private static final int N_GRADIENTS_4D_MULTIPLIER = 5;
    private static final int N_GRADIENTS_4D_MULTIPLIER_COVERAGE_EXPONENT = 3;
    private static final int N_GRADIENTS_4D = (1 << N_GRADIENTS_4D_BASE_EXPONENT) * N_GRADIENTS_4D_MULTIPLIER;
    private static final int GRADIENTS_4D_BITS_CONSIDERED = 31 - N_GRADIENTS_4D_MULTIPLIER_COVERAGE_EXPONENT;
    private static final int GRADIENTS_4D_HASH_MASK = (1 << GRADIENTS_4D_BITS_CONSIDERED) - 1;
    private static final int GRADIENT_VECTOR_4D_LENGTH_EXPONENT = 2;
    private static final int GRADIENT_VECTOR_4D_LENGTH = 1 << GRADIENT_VECTOR_4D_LENGTH_EXPONENT;
    private static final int GRADIENTS_4D_BIT_SHIFT = GRADIENTS_4D_BITS_CONSIDERED - N_GRADIENTS_4D_BASE_EXPONENT - GRADIENT_VECTOR_4D_LENGTH_EXPONENT;
    private static final int GRADIENTS_4D_SELECTION_MASK = ((1 << (N_GRADIENTS_4D_BASE_EXPONENT + N_GRADIENTS_4D_MULTIPLIER_COVERAGE_EXPONENT)) - 1) << GRADIENT_VECTOR_4D_LENGTH_EXPONENT;
    ...
    private static float grad(long seed, long xsvp, long ysvp, long zsvp, long wsvp, float dx, float dy, float dz, float dw) {
        ...
        hash *= HASH_MULTIPLIER;
        hash ^= hash >> (64 - GRADIENTS_4D_BITS_CONSIDERED);

        int index = (int)hash & GRADIENTS_4D_HASH_MASK;
        index = (index * N_GRADIENTS_4D_MULTIPLIER) >> GRADIENTS_4D_BIT_SHIFT;
        index &= GRADIENTS_4D_SELECTION_MASK;

        return (GRADIENTS_4D[index | 0] * dx + GRADIENTS_4D[index | 1] * dy) + (GRADIENTS_4D[index | 2] * dz + GRADIENTS_4D[index | 3] * dw);
    }
    ...
    static {
        ...
        for (int i = 0; i < GRADIENTS_2D.length; i++) {
            GRADIENTS_2D[i] = (float)(grad2[i] / NORMALIZER_2D);
        }
        ...
        GRADIENTS_4D = new float[N_GRADIENTS_4D * GRADIENT_VECTOR_4D_LENGTH];
        double[] grad4 = {
        ...
        for (int i = 0; i < GRADIENTS_4D.length; i++) {
            GRADIENTS_4D[i] = (float)(grad4[i] / NORMALIZER_4D);
        }
