...
public class OpenSimplex2F_Shorter4DCode {
	...
	private double noise4_Base(double xs, double ys, double zs, double ws) {
		...
		// We only look at the spaces between the diagonal planes. This produced no cutoffs in my tests.
		double siSum = xsi + ysi + zsi + wsi;
		int startingLattice = (int)(siSum * 1.25);
		
		// Offset for lattice hashing coordinates
		int startingLatticeOffsetB = startingLattice * 409;
		xsb -= startingLatticeOffsetB; ysb -= startingLatticeOffsetB; zsb -= startingLatticeOffsetB; wsb -= startingLatticeOffsetB;
		
		// Offset for lattice point relative positions (skewed)
		double startingLatticeOffset = startingLattice * -0.2;
		...
		double ssi = (siSum + startingLatticeOffset * 4) * 0.309016994374947;
		...
		for (int i = 0; i < 5; i++) {
			...
			// Reset hashing coordinates we address the lattices in different orders
			if (i == startingLattice) {
				xsb -= 2046; ysb -= 2046; zsb -= 2046; wsb -= 2046;
			}
